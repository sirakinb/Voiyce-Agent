// This function is deployed as a single source file by the InsForge CLI, so
// keep its error-safety helpers local rather than importing a sibling module.
const GENERIC_CLIENT_ERROR = 'The request failed. Please try again.'
const AI_SERVICE_UNAVAILABLE_ERROR = 'The AI service is temporarily unavailable. Please try again.'
const ACCOUNT_USAGE_LIMIT_ERROR = 'This account has reached its current usage limit. Try again later or contact support if this seems wrong.'
...
function safeClientMessage(message: string, fallback = GENERIC_CLIENT_ERROR): string {
...
function isAccountUsageLimitMessage(message: string): boolean {
...
function accountUsageLimitMessage(message: string): string {
