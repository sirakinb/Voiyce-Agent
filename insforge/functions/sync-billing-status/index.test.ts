...
Deno.test("billing sync blocks Stripe live mode unless explicitly acknowledged", async () => {
  clearStripeEnv()
  ...
  try {
    const response = await handler(new Request("https://functions.test/sync-billing-status", {
      method: "POST",
      headers: {
